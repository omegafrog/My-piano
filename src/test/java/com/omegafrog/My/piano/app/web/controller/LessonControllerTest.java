...
class LessonControllerTest {
  ...
  private MockHttpSession artistSession;
  private MockHttpSession otherSession;
  private MockHttpSession otherSession2;
  @Autowired
  ...
  void createLessonTest() throws Exception {
    ...
    artistSession = testUtil.login(mockMvc, TestUtil.user1.getUsername(), TestUtil.user1.getPassword());
    // register other user
    testUtil.register(mockMvc, TestUtil.user2);
    otherSession = testUtil.login(mockMvc, TestUtil.user2.getUsername(), TestUtil.user2.getPassword());

    testUtil.register(mockMvc, TestUtil.user3);
    otherSession2 = testUtil.login(mockMvc, TestUtil.user3.getUsername(), TestUtil.user3.getPassword());

    RegisterSheetPostDto registerSheetPostDto = TestUtil.registerSheetPostDto(TestUtil.registerSheetDto1);
    SheetPostDto sheetPostDto = testUtil.writeSheetPost(mockMvc, artistSession, registerSheetPostDto);
  ...
  void updateLessonTest() throws Exception {
    ...
    SheetPostDto sheetPostDto = testUtil.writeSheetPost(mockMvc, artistSession, registerSheetPostDto);
    RegisterLessonDto registerLessonDto = TestUtil.registerLessonDto(sheetPostDto.getSheet().getId());
    LessonDto lessonDto = testUtil.writeLesson(mockMvc, artistSession, registerLessonDto);
  ...
  void deleteCommentTest() throws Exception {
    ...
    String contentAsString2 = mockMvc.perform(delete("/api/v1/lessons/" + lessonDto.getId() + "/comments/" + commentId)
        .session(otherSession))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value(HttpStatus.OK.value()))
  ...
  void findLessonTest() throws Exception {
    ...
    LessonDto savedLessonDto = testUtil.writeLesson(mockMvc, artistSession, registerLessonDto);
  ...
  void findAllLessonTest() throws Exception {
    ...
    SheetPostDto sheetPostDto1 = testUtil.writeSheetPost(mockMvc, artistSession, registerSheetPostDto1);
    SheetPostDto sheetPostDto2 = testUtil.writeSheetPost(mockMvc, artistSession, registerSheetPostDto2);
    ...
    LessonDto savedLessonDto1 = testUtil.writeLesson(mockMvc, artistSession, registerLessonDto1);
    LessonDto savedLessonDto2 = testUtil.writeLesson(mockMvc, artistSession, registerLessonDto2);
