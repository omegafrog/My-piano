...
public class RegisterSheetDto {
	...
	public Sheet createEntity(User user, int pageNum) {
		return Sheet.builder()
			.title(title)
			.difficulty(
				Arrays.stream(Difficulty.values()).filter(iter -> iter.ordinal() == difficulty).findFirst().get())
			.instrument(
				Arrays.stream(Instrument.values()).filter(iter -> iter.ordinal() == instrument).findFirst().get())
			.genres(genres)
			.isSolo(isSolo)
			.lyrics(lyrics)
			.user(user)
