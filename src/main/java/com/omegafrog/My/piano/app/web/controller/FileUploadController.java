...
public class FileUploadController {
    ...
    public JsonAPIResponse<FileUploadResponse> uploadFile(
        ...
        if (response.getStatus() == UploadOutboxEventStatus.FAILED) {
    ...
    public JsonAPIResponse<UploadOutboxEventStatus> getUploadStatus(
            @Valid @NotNull @PathVariable String uploadId) {

        log.info("Upload status request for uploadId: {}", uploadId);

        UploadOutboxEventStatus status = fileUploadService.getUploadStatus(uploadId);
