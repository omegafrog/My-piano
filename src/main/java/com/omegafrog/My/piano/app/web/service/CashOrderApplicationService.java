...
public class CashOrderApplicationService {
  ...
  public Page<PaymentHistory> getPaymentHistory(LocalDate start, LocalDate end, Pageable pageable) {
    User loggedInUser = authenticationUtil.getLoggedInUser();
    // expired handling
    List<CashOrder> expired = cashOrderRepository.findExpired(loggedInUser.getId(),
        new CustomDateRange(LocalDate.now(), LocalDate.now()));
    for (CashOrder order : expired)
      order.changeState(OrderStatus.EXPIRED);

    Page<CashOrder> cashOrders;
    if (start != null && end != null)
      cashOrders = cashOrderRepository.findByUserIdAndDate(loggedInUser.getId(), pageable,
          dateRangeFactory.calcDateRange(start, end));

    else
      cashOrders = cashOrderRepository.findByUserId(loggedInUser.getId(), pageable);
  ...
  public void handleExpiredCashOrders() {
    ...
    List<CashOrder> expiredCashOrders = cashOrderRepository.findExpired(new CustomDateRange(today, today));
